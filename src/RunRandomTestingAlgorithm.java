...
public class RunRandomTestingAlgorithm {
	...
	public RunRandomTestingAlgorithm(ArrayList<Query> random, QDataset d) throws IOException
	{
		this.random = random;
	...
	{
		String[] args1 = {"--cmd=generate-bin", "--ranking", "--folder", "src/data/LETOR/forEval/randomQ/", "--file", "forEval_random_train.txt"/*, "--file", "valid.txt", "--file", "test.txt"*/};
		return args1;
	}
	
	String[] buildArgsForRanking()
	{
		String[] args1 = {"--cmd=train", "--ranking", "--config-file", "src/data/ranking.properties", "--train-file", "src/data/LETOR/forEval/randomQ/forEval_random_train.bin", "--validation-file", "src/data/LETOR/forEval/valid.bin", "--output-model", "src/data/LETOR/forEval/randomQ/forEval_random_ensemble.txt"};
		return args1;
	}
	
	String[] buildArgsForPredicting()
	{
		String[] args1 = {"--cmd=predict", "--ranking", "--model-file", "src/data/LETOR/forEval/randomQ/forEval_random_ensemble.txt", "--tree-type", "RegressionTree", "--test-file", "src/data/LETOR/forEval/test.bin", "--output-file", "src/data/LETOR/forEval/randomQ/forEval_random_predictions.txt"};
		return args1;
	}
