...
public class RunLearningAlgorithmRankLib {
	...
	public RunLearningAlgorithmRankLib(ArrayList<Query> subset, int i)
	{
		...
		String args1[] = {"-train", "src/data/LETOR/train"+i+".txt", "-test", "src/data/LETOR/candidate.txt", "-validate", "src/data/LETOR/valid.txt", "-ranker", "2", "-metric2t", "NDCG@10", "-metric2T", "NDCG@10", "-save", "src/data/LETOR/mymodel"+i+".txt"};
		runAlgo(args1);
		String args2[] = {"-load", "src/data/LETOR/mymodel"+i+".txt", "-rank", "src/data/LETOR/candidate.txt", "-score", "src/data/LETOR/predictions"+i+".txt"};
		runAlgo(args2);
		deleteBinFiles();
	}
